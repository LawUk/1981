...
/**
 * Lookbook Dinámico con Volúmenes
 * Lee imagenes.json (array de volúmenes) y renderiza cada volumen
 * con su separador de título y sus fotos.
 *
 * Estructura de imagenes.json:
 * [
 *   { "volumen": "Vol. 2 — 2026", "carpeta": "lookbook/2026", "fotos": ["a.png"] },
 *   { "volumen": "Vol. 1 — 2025", "carpeta": "lookbook/2025", "fotos": ["b.png"] }
 * ]
 *
 * Para agregar fotos nuevas:
 * 1. Sube la imagen a assets/images/lookbook/2026/
 * 2. Agrega su nombre al array "fotos" del volumen 2026 en imagenes.json
 */
  ...
  fetch(basePath('imagenes.json'))
    ...
    .then(volumenes => {
      ...
      volumenes.forEach(vol => {
        ...
        // Si el volumen no tiene fotos, no lo mostramos
        if (!vol.fotos || vol.fotos.length === 0) return;

        // Separador de volumen
        ...
        vol.fotos.forEach(nombreImagen => {
          ...
          const imagePath = `./assets/images/${vol.carpeta}/${nombreImagen}`;

          img.src     = imagePath;
          img.alt     = `Lookbook ${vol.volumen}`;
          img.loading = "lazy";
      ...
      // Si no había ningún volumen con fotos, mostrar mensaje
  ...
  fetch(basePath('blog.json'))
    ...
    .then(entradas => {
      ...
      primeras.forEach(entrada => {
        ...
        const linkHref = entrada.link || `./entrada.html?id=${entrada.id}`;
        ...
        otras.forEach(otra => {
          ...
          article.innerHTML = `
            ...
            <div class="blog-card-footer">
              ...
              <a href="./entrada.html?id=${otra.id}" class="blog-card-link">
                Leer
                <ion-icon name="arrow-forward-outline" aria-hidden="true"></ion-icon>
              </a>
            </div>
